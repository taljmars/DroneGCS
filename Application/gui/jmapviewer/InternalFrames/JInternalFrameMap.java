...
import gui.jmapviewer.JMapViewerTree;
import gui.jmapviewer.Layer;
import gui.jmapviewer.LayerGroup;
import gui.jmapviewer.LayerMission;
import gui.jmapviewer.LayerPerimeter;
import gui.jmapviewer.MapLineImpl;
import gui.jmapviewer.MapMarkerCircle;
import gui.jmapviewer.MapMarkerDot;
import gui.jmapviewer.MapObjectImpl;
import gui.jmapviewer.MapPathImpl;
import gui.jmapviewer.MapPolygonImpl;
import gui.jmapviewer.MapRectangleImpl;
import gui.jmapviewer.MyStroke;
import gui.jmapviewer.OsmTileLoader;
...
import gui.jmapviewer.tilesources.MapQuestOpenAerialTileSource;
import gui.jmapviewer.tilesources.MapQuestOsmTileSource;
import gui.jmapviewer.tilesources.OsmTileSource;
...
import org.droidplanner.core.MAVLink.MavLinkLand;
...
public class JInternalFrameMap extends JInternalFrame implements JMapViewerEventListener, OnDroneListener, OnWaypointManagerListener {
        ...
        cbLockMyPos.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cbLockMyPos.isSelected()) {
					Dashboard.addGeneralMessegeToDisplay("Lock on my position");
					lockMapOnMyPosition = true;
				}
				else {
					Dashboard.addGeneralMessegeToDisplay("Release lock on my position");
        ...
        cbFollowTrail.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cbFollowTrail.isSelected()) {
					Dashboard.addGeneralMessegeToDisplay("Paint My Trail");
					myTrailPath = null;
					paintTrail = true;
				}
				else {
					Dashboard.addGeneralMessegeToDisplay("Stop Paint My Trail");
					paintTrail = false;
	...
	public JPopupMenu createPopupMenu(MouseEvent e) {		
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (!GuidedPoint.isGuidedMode(Dashboard.drone)) {
                	...
                	if (n == JOptionPane.YES_OPTION) {
                		...
                		//Dashboard.addGeneralMessegeToDisplay("Flight Mode changed to '" + Dashboard.drone.getState().getMode().getName() + "'");
                	}
                	else {
                		return;
                	}
                ...
                try {
                	...
                	Dashboard.addGeneralMessegeToDisplay("Flying to guided point " + guidedPoint.getCoordinate().toString());
                } 
                catch (Exception e1) {
        ...
        menuItemSyncMission.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
            	...
            	Dashboard.addOutgoingMessegeToDisplay("Send Sync Request");
			}
		});
            ...
            public void actionPerformed(ActionEvent arg0) {
            	...
            	switch (n) {
            		case 0:
            			...
            			Dashboard.addGeneralMessegeToDisplay("Start GeoFence of manual circle type");
            			break;
            		case 1:
            			setGeoFenceByMouse = true;
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and mouse roller to set radius");
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and mouse roller to set radius");
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and mouse roller to set radius");
            			updateCycleGeoFence(radi, iCoord);
            			Dashboard.addGeneralMessegeToDisplay("Start GeoFence of fixed circle type");
            			break;
            		case 2:
            			setPerimeterByMouse = true;
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and left mouse key to add point");
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and left mouse key to add point");
            			Dashboard.MessegeQueue_Add("Use Ctrl Key and left mouse key to add point");
            			map().removeMapMarker(perimeterBreachPointMarker);
            			perimeterBreachPointMarker = null;
            			Dashboard.addGeneralMessegeToDisplay("Start GeoFence of perimeter type");
	...
	public void SetHome(Home home) {
		...
		Dashboard.addGeneralMessegeToDisplay("Setting new Home position");
	}
	...
	private void EditModeOff() {
		...
		Dashboard.addGeneralMessegeToDisplay("Edit mode is off");
	}
	
	private void EditModeOn() {
		Dashboard.addGeneralMessegeToDisplay("Edit mode is on");
	}
	...
	public void onBeginWaypointEvent(WaypointEvent_Type wpEvent) {
		if (wpEvent.equals(WaypointEvent_Type.WP_DOWNLOAD)) {
			Dashboard.addIncommingMessegeToDisplay("Start Syncing");
			return;
		}
		if (wpEvent.equals(WaypointEvent_Type.WP_UPLOAD)) {
			Dashboard.addIncommingMessegeToDisplay("Start Updloading Waypoints");
			return;
		}
		
		Dashboard.addIncommingMessegeToDisplay("Failed to Start Syncing (" + wpEvent.name() + ")");
	}

	@Override
	public void onWaypointEvent(WaypointEvent_Type wpEvent, int index, int count) {
		if (wpEvent.equals(WaypointEvent_Type.WP_DOWNLOAD)) {
			Dashboard.addIncommingMessegeToDisplay("Downloading Waypoint " + index + "/" + count);
			Dashboard.window.setProgressBar(0, index, count);
			return;
		}
		
		if (wpEvent.equals(WaypointEvent_Type.WP_UPLOAD)) {
			Dashboard.addIncommingMessegeToDisplay("Uploading Waypoint " + index + "/" + count);
			Dashboard.window.setProgressBar(0, index, count);
			return;
		}
		
		Dashboard.addErrorMessegeToDisplay("Unexpected Syncing Failure (" + wpEvent.name() + ")");
	...
	public void onEndWaypointEvent(WaypointEvent_Type wpEvent) {
		if (wpEvent.equals(WaypointEvent_Type.WP_DOWNLOAD)) {
			Dashboard.addIncommingMessegeToDisplay("Waypoints Synced");
			if (Dashboard.drone.getMission() == null) {
				Dashboard.addIncommingMessegeToDisplay("Failed to find mission");
				return;
			}
			...
			Dashboard.addIncommingMessegeToDisplay("Current mission was loaded to a new view");
			return;
		}
		...
		Dashboard.addErrorMessegeToDisplay("Failed to Sync Waypoints (" + wpEvent.name() + ")");
	}
		...
		switch (event) {
			...
			case GCS_LOCATION:
				if (drone.getGCS().getPosition() == null) {
					Dashboard.addErrorMessegeToDisplay("GCS location doesn't exist");
					return;
				}
	...
	private void UpdateGCSOnMap(Coord2D coord) {
		...
		Dashboard.addGeneralMessegeToDisplay("GCS was updated");
	}
	...
	private void UpdateBeaconOnMap(Coordinate coord) {
		...
		Dashboard.addGeneralMessegeToDisplay("Beacon was updated");
	}
