...
public class KeyBoardControllerImpl implements KeyBoardController, Runnable {
	...
	private boolean threadKeyboard = true;
	...
	public void init() {
		...
		keyboardStabilizer = new Thread(this, "KeyBoardControllerThread");
		keyboardStabilizer.start();
	}

	@PreDestroy
	public void destroy() {
		threadKeyboard = false;
	}
	...
	public void run() {
		...
		logger.LogGeneralMessege(this.getClass().getName() + " Stabilizer Thread stop");
	}
	...
	public void Reset() {
		logger.LogGeneralMessege("Reseting RC Set");
		ResetRCSet();
	}
