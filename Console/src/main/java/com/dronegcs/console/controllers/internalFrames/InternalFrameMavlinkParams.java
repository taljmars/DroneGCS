					...
					if ( !empty && getIndex() > 0 ) {
						btn.setOnAction( ( ActionEvent event ) -> {
							...
							if (value != null) {
								...
								Parameter parameter = new Parameter(entry.getName(), value, entry.getType());
	...
	public void loadTable() {
		...
		if (parametersList.size() > 0) {
			...
			for (int i = 0; i < parametersList.size(); i++) {
				...
				ParamsTableEntry entry = new ParamsTableEntry(i, parameter.name, parameter.getValue(), parameter.type);
				data.add(entry);
			}
			table.setItems(data);
