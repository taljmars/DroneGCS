...
import gui.jmapviewer.interfaces.MapMarker;
import gui.jmapviewer.interfaces.MapObject;
import gui.jmapviewer.interfaces.MapPath;
import gui.jmapviewer.interfaces.MapPolygon;
import gui.jmapviewer.interfaces.MapRectangle;
import gui.jmapviewer.interfaces.TileCache;
import gui.jmapviewer.interfaces.TileLoader;
...
public class JMapViewer extends JPanel implements TileLoaderListener, MouseListener {
    ...
    public JMapViewer(TileCache tileCache) {
        ...
        mapLineList = Collections.synchronizedList(new LinkedList<MapLine>());
        mapRectangleList = Collections.synchronizedList(new LinkedList<MapRectangle>());
        mapPathList = Collections.synchronizedList(new LinkedList<MapPath>());
        mapMarkersVisible = true;
        mapRectanglesVisible = true;
        mapLinesVisible = true;
        mapPolygonsVisible = true;
        mapPathsVisible = true;
	...
	public Point ConvertToPoint(Coordinate point) {
		return tileSource.latLonToXY(point, getZoom());
	}
    ...
    protected void initializeStatusLabel() {
    	btnEditSaveMode = new JButton("Save");
    	btnEditSaveMode.setForeground(Color.RED);
    	btnEditSaveMode.setBackground(Color.RED);
    	btnEditSaveMode.setBounds(4, 180, 90, 25);
    	btnEditSaveMode.setOpaque(false);
    	...
    	btnEditCancelMode.setForeground(Color.RED);
    	btnEditCancelMode.setBackground(Color.RED);
    	btnEditCancelMode.setBounds(4, 210, 90, 25);
    	btnEditCancelMode.setOpaque(false);
    ...
    public void SetEditModeGUI(boolean set_on) {
    	if (set_on) {
    		System.out.println(getClass().getName() + " Setting buttons visible");
    		btnEditSaveMode.setEnabled(true);
    		btnEditSaveMode.setVisible(true);
    		btnEditCancelMode.setEnabled(true);
    		btnEditCancelMode.setVisible(true);
    	}
    	else {
    		System.out.println(getClass().getName() + " Setting buttons invisible");
    		btnEditSaveMode.setEnabled(false);
    		btnEditSaveMode.setVisible(false);
    		btnEditCancelMode.setEnabled(false);
    		btnEditCancelMode.setVisible(false);
     ...
     */
    public void setDisplayToFitMapElements(boolean markers, boolean rectangles, boolean polygons, boolean lines, boolean paths) {
        int nbElemToCheck = 0;
        ...
        if (paths && mapPathList != null)
            nbElemToCheck += mapPathList.size();
        ...
        if (lines && mapLineList != null) {
            synchronized (mapLineList) {
                for (MapLine line : mapLineList) {
                    if (line.isVisible()) {
                    	Point start = tileSource.latLonToXY(line.getStart(), mapZoomMax);
                        Point end = tileSource.latLonToXY(line.getEnd(), mapZoomMax);
                        xMax = Math.max(xMax, start.x);
                        yMax = Math.max(yMax, end.y);
                        xMin = Math.min(xMin, end.x);
                        yMin = Math.min(yMin, start.y);
                    }
                }
            }
        }
        ...
        if (paths && mapPathList != null) {
            synchronized (mapPathList) {
                for (MapPath path : mapPathList) {
                    if (path.isVisible()) {
                    	for (ICoordinate c : path.getPoints()) {
                            Point p = tileSource.latLonToXY(c, mapZoomMax);
                            xMax = Math.max(xMax, p.x);
                            yMax = Math.max(yMax, p.y);
                            xMin = Math.min(xMin, p.x);
     ...
     * Sets the displayed map pane and zoom level so that all map polygons are visible.
     */
    public void setDisplayToFitMapPaths() {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (mapPathsVisible && mapPathList != null) {
            synchronized (mapPathList) {
                for (MapPath path : mapPathList) {
                    if (path.isVisible())
                        paintPath(g, path);
    ...
    protected void paintLine(Graphics g, MapLine line) {
        ...
        if (start != null && end != null) {
            Point pStart = getMapPosition(start, false);
            Point pEnd = getMapPosition(end, false);
            if (pStart != null && pEnd != null) {
            	line.paint(g, pStart, pEnd);
                if (scrollWrapEnabled) {
                    int tilesize = tileSource.getTileSize();
                    int mapSize = tilesize << zoom;
                    int xTopLeftSave = pStart.x;
                    int xTopLeftWrap = xTopLeftSave;
                    int xBottomRightSave = pEnd.x;
                    int xBottomRightWrap = xBottomRightSave;
                    while ((xBottomRightWrap -= mapSize) >= 0) {
                        xTopLeftWrap -= mapSize;
                        pStart.x = xTopLeftWrap;
                        pEnd.x = xBottomRightWrap;
                        line.paint(g, pStart, pEnd);
                    }
                    xTopLeftWrap = xTopLeftSave;
                    xBottomRightWrap = xBottomRightSave;
     ...
     * @param polygon polygon to paint
     */
    protected void paintPath(Graphics g, MapPath path) {
        List<? extends ICoordinate> coords = path.getPoints();
        if (coords != null && coords.size() >= 3) {
            List<Point> points = ConvertICoordinatesToPoints(coords);
    ...
    /**
     * Sets the list of {@link MapPolygon}s.
     * @param mapPathList list of {@link MapPolygon}s
     */
    public void setMapPathList(List<MapPath> mapPathList) {
     ...
     * @return list of {@link MapPolygon}s
     */
    public List<MapPath> getMapPathList() {
        return mapPathList;
    }
     ...
     * @param polygon map polygon to add
     */
    public void addMapPath(MapPath path) {
     ...
     * @param polygon map polygon to remove
     */
    public void removeMapPath(MapPath path) {
    	mapPathList.remove(path);
        repaint();
    }
     ...
     * @return {@code true} if the {@link MapPolygon}s are painted, {@code false} otherwise
     */
    public boolean isMapPathsVisible() {
        return mapPathsVisible;
    }
     ...
     * @see #getMapPolygonList()
     */
    public void setMapPathsVisible(boolean mapPathsVisible) {
        this.mapPathsVisible = mapPathsVisible;
        repaint();
    }
	...
	public void updateKeboardEvent(KeyEvent keyEvent) {
	...
	public List<Point> ConvertICoordinatesToPoints(List<? extends ICoordinate> coords) {
		List<Point> points = new LinkedList<>();
		if (coords == null) {
			Logger.LogErrorMessege(getClass() + ": Received empty coordinate set");
			return points;
		}
	...
	public void removeMapObject(MapObject mo) {		
		if (mo instanceof MapPath) {
			this.removeMapPath((MapPath) mo);
			return;
		}
		
		if (mo instanceof MapMarker) {
			this.removeMapMarker((MapMarker) mo);
			return;
		}
		
		if (mo instanceof MapPolygon) {
			this.removeMapPolygon((MapPolygon) mo);
			return;
		}
		
		if (mo instanceof MapLine) {
			this.removeMapLine((MapLine) mo);
			return;
		}
		
		if (mo instanceof MapRectangle) {
			this.removeMapRectangle((MapRectangle) mo);
			return;
		}
		
		System.out.println(getClass().getName() + " Failed to remove Map Object");
	}

	public void addMapObject(MapObject mapObject) {
		if (mapObject instanceof MapMarker) {
			addMapMarker((MapMarker)mapObject);
			return;
		}
		
		if (mapObject instanceof MapPolygonImpl) {
			addMapPolygon((MapPolygonImpl)mapObject);
			return;
		}
		
		if (mapObject instanceof MapPathImpl) {
			addMapPath((MapPathImpl)mapObject);
			return;
		}
		
		if (mapObject instanceof MapLineImpl) {
			addMapLine((MapLineImpl)mapObject);
			return;
		}
		
		if (mapObject instanceof MapRectangle) {
			addMapRectangle((MapRectangle)mapObject);
			return;
		}
		
		System.out.println(getClass().getName() + " Failed to add Map Object");
	}
	
	public void RemoveLayersDrawings(){
		synchronized (mapLineList) {
            for (MapLine line : mapLineList) {
               removeMapLine(line);
            }
		...
		synchronized (mapPolygonList) {
            for (MapPolygon poly : mapPolygonList) {
		...
		synchronized (mapRectangleList) {
            for (MapRectangle rec : mapRectangleList) {
               removeMapRectangle(rec);
            }
        }
		repaint();
    }
