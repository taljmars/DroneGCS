...
public class MapTile {
    private final static Logger LOGGER = LoggerFactory.getLogger(MapTile.class);
    ...
    public MapTile(TileSource source, int xtile, int ytile, int zoom) {
        ...
        if (this.tileLocation.startsWith("http")) {
            ...
            LOGGER.error("Unknown Tile [" + this.tileLocation + "]");
        }
    ...
    private void loadImage(String url, boolean localImage) {
        ...
        img.progressProperty().addListener((ObservableValue<? extends Number> ov, Number old_val, Number new_val) -> {
            try {
                ...
                LOGGER.error("Failed to save tile");
                e.printStackTrace();
            }
        });
    ...
    private void saveToDisk(Image img, String fileName) throws URISyntaxException {
        ...
        LOGGER.info("Tile '{}' not exist, saving to disk", file);
        BufferedImage bImage = SwingFXUtils.fromFXImage(img, null);
        try {
            ImageIO.write(bImage, extention, file);
        } catch (IOException e) {
            LOGGER.error("Failed to dump tile");
            e.printStackTrace();
        }
    }
